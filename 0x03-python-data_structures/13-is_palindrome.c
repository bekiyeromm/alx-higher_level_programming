 ...
 * @c: integer count of how many nodes from head to check the end
 * Return: 1 if palindrome, 0 if not
 */

int palindrome_check(listint_t *tmp_head, int c)
{
	listint_t *tmp_end = tmp_head;
	int count = 0;
	/* move tmp_end to the end of where checking based on input count c */
	...
	c = c - 2;
	return (palindrome_check(tmp_head, c));
}
